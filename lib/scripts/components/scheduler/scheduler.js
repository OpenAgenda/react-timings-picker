...
var Scheduler = React.createClass({displayName: "Scheduler",
  ...
  dayMouseMove: function (e) {
    ...
    if ( y < 0 ) {
      ...
      if ( newValue < userActionValues.minDragY + 2 ) {
        ...
        top = userActionValues.minDragY - parentOffsetTop + 2 + this.state.canvasScroll;
        height = userActionValues.initialY - top - parentOffsetTop, userActionValues.dragStep;
        start = userActionValues.minTime;

      } else {

        top = userActionValues.initialTop + y + this.state.canvasScroll;
        height = Math.abs(y);
        start = utils.addMinutes( actionTiming.start, minDiff );
      ...
      top = userActionValues.initialTop + this.state.canvasScroll;
  ...
  render: function () {
    ...
    return (
      React.createElement("div", {className: "rc-scheduler", ref: "scrollable"}, 
