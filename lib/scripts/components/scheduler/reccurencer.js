	...
	render: function () {
		...
		if (this.state.createReccurence == true) {
			result = (React.createElement("div", null, 
				...
				React.createElement(DatePicker, {selected: this.state.startDate, onChange: this.onReccurenceStartChange, weekdays: strings.weekdays.short, months: strings.months}), " ", strings.to, 
				React.createElement(DatePicker, {selected: this.state.endDate, onChange: this.onReccurenceEndChange, weekdays: strings.weekdays.short, months: strings.months}), 
				React.createElement("a", {className: "rc-ok-button", onClick: this.createReccurences}, "OK")
			));
		}
		else {
